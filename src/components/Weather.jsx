...
const Weather = () => {
  ...
  function getWeather() {
    var weatherData = axios(
      `https://api.openweathermap.org/data/2.5/weather?q=${cityInput}&appid=429634b49a952b316f23897e35089a0e`
    )
          ...
          {visible && (
            <div className="bg-white/10 rounded p-4 space-y-2 text-black backdrop-blur-sm">
              ...
              <p className='flex gap-2 items-center text-xl'>
                ...
                <span className="font-semibold text-xl">Weather:</span> {weather}
              </p>
              <p className='flex gap-2 items-center text-xl'>
                <FaTemperatureHigh className="text-red-400" />
                <span className="font-semibold text-xl">Temperature:</span> {temp}°C
              </p>
              <p className='flex gap-2 items-center text-xl'>
                <FaRegStickyNote className="text-blue-300" />
                <span className="font-semibold text-xl">Description:</span> {desc}
              </p>
            </div>
          )}
        </div>
